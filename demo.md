...
|---|---|---|---|
| C1 | Install & boot | `docker compose up -d` (postgres must be **`pgvector/pgvector:pg15`** — plain postgres fails migration 01) + migrations auto-run | `python scripts/migrate.py --status` → all 33 `applied` (33 files in `backend/db/` as of 2026-09-02, up from 30 — file count verified by directory listing; not re-run against a live DB this pass, no Postgres instance available); first verified engine run 2026-08-25 (`research/claude-code-hooks-v2@429ffa9`) |
