...
|---|---|---|---|
| C1 | Install & boot | `docker compose up -d` (postgres must be **`pgvector/pgvector:pg15`** — plain postgres fails migration 01) + migrations auto-run | `python scripts/migrate.py --status` → all 33 `applied` (33 files in `backend/db/` as of 2026-09-02); re-run this pass against a genuinely fresh throwaway `pgvector/pgvector:pg15` container — 33/33 applied, 0 pending, 0 checksum mismatches, 0 errors (engine-verified 2026-09-02); first verified engine run 2026-08-25 (`research/claude-code-hooks-v2@429ffa9`) |
