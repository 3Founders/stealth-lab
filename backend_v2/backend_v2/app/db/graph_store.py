...
class GraphStore:
    ...
    async def traverse_from(
        ...
        """
        Expand outward from entrypoints up to `max_depth` hops. Shallow by
        design; not an exhaustive graph walk.

        UNION dedupes the frontier and the depth bound guarantees
        termination on cyclic graphs.

        `max_edges` bounds what is returned, not what the CTE does: Postgres
        forbids LIMIT in a recursive term, so breadth can't be capped
        mid-recursion. It stops a hub node from flooding callers, and the
        truncation is logged rather than silent. Bounding the work itself
        needs a materialised degree column.
        """
        if not entrypoint_ids:
            return []
        cap = max_edges or settings.max_traversal_edges
        ...
        if len(rows) >= cap:
            log.warning(
                "traversal hit the %d-edge cap from %d entrypoint(s) at depth %d; "
                "results truncated", cap, len(entrypoint_ids), max_depth,
            )
        return [_row_to_edge(r) for r in rows]
