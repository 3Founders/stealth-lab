...
DEFAULT_LIMITS: dict[str, RateLimit] = {
    ...
    # Two model calls (generate + critique), ~3x a chat message.
    "/v1/decompose": RateLimit(max_requests=10, window=timedelta(hours=1)),
}
...
class MeteredAgent:
    """
    Wraps a PanelAgent so its calls land in `llm_spend`.

    `check_budget` sums that table, so a budget nothing records against
    never binds: the check reads zero and passes forever. Wiring the
    record half is what makes the cap real rather than decorative -- the
    same failure the access predicate in `services/access.py` was
    centralised to avoid, one layer over.

    Metering wraps the agent rather than the endpoint because one request
    fans out: a scan spends on three panelists per round, a judge per
    candidate, and one call per Layer 2 simulation. Recording at the
    handler would count one call and miss dozens.

    Tokens are estimated, not measured. The PanelAgent protocol returns
    text, not the provider's usage metadata, so `estimate_tokens` is the
    available signal. Good enough for a guardrail, not good enough to
    bill from.
    """
    ...
    def __post_init__(self) -> None:
        # Proxied, not re-derived: assert_heterogeneous and
        # enforce_independence compare `family`, so a wrapper that hid it
        # would silently disable the panel-independence checks.
        self.agent_id = self.inner.agent_id
        self.model_id = self.inner.model_id
        self.family = self.inner.family

    async def respond(self, system: str, user: str) -> str:
        output = await self.inner.respond(system, user)
        # A local server costs nothing, but its family name (llama, qwen,
        # mistral) is absent from the price table, and estimate_cost
        # charges unknown providers at the worst-case rate. Without this,
        # a free local run would accumulate fake spend and trip the cap.
        provider = "local" if settings.use_local_models else self.family
        await CostGovernor(self.pool).record(
            provider=provider,
            model=self.model_id,
