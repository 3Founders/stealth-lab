...
from app.services.governance import MeteredAgent
...
async def run_scan(
    ...
    def metered(agent, operation: str):
        return MeteredAgent(agent, pool, operation=operation, scope_key=scope_key)

    try:
        # Every seat is metered separately: one scan is three panelists per
        # round, a judge per candidate, and a call per Layer 2 simulation.
        # The operation label is what makes that breakdown readable in
        # llm_spend afterwards.
        orchestrator = LoopOrchestrator(
            pool,
            [metered(a, "debate") for a in default_panel()],
            metered(default_judge(), "judge"),
            layer2_agent=metered(default_layer2_agent(), "layer2"),
        )
