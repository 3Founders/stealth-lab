...
- **T2** — no per-migration fresh-DB / representative-row / idempotency / rollback tests for 50–55; `test_schema_drift.py` not run against the DB.
- **T3** — no end-to-end golden ingestion test (a real SKILL.md → assert the full object chain against Postgres). Now writable (the chain exists); needs DB egress.
- **A33 corpus backfill** — `backfill_refs_from_preconditions` still not run against real data.
- **B15** — `ingestion_jobs.py` hardcoded placeholder goal + unconditional `outcome="success"` in the trace→procedure sweep, untouched.
- **B9 partial** — `failure_handlers.py` still writes `claim_status` directly.
- **B1 residual** — block-level secret redaction; citing the document Observation to a specific block span.
- **G9 / G12 / G13 / G14** — procedure-composition cycle rejection, local/private sync, `.stealth/` projection contract, retrieval index-lag tracking — none touched.
...
Release-critical unless noted. `CLOSED` = spec requirement met **and** DB/E2E-verified. `CODE-COMPLETE (T2/T3-UNVERIFIED)` = migration applied + writer wired + offline proving tests green, but no per-migration T2 test and no DB golden-E2E (T3) has run. `PARTIAL` = substantial mechanism exists with a named gap. `OPEN` = not built / not started. Migrations 50–55 are **applied** (01–55, 0 pending, 0 mismatch).
...
| **G1** | `IngestionContext` / provenance manifest | **CODE-COMPLETE (T2/T3-UNVERIFIED)** | mig 51 `ingestion_contexts` (applied) + `ingestion_context.py` + back-links on 5 tables; `compile_skill_artifact` opens/completes one and stamps it on procedure/observation/evidence/artifact/blocks/claim. Remaining: thread through the trace path; T2. |
| **G2** | Source + Artifact normalization + immutable block addressing | **CODE-COMPLETE (T2/T3-UNVERIFIED)** | mig 50 `sources` + mig 55 `artifact_blocks` (applied); `sources.py`; `normalize_markdown` **now invoked** by `compile_skill_artifact` → `persist_artifact_blocks`. Remaining: cite the document Observation to a specific block span; a `Source [V]` for the trace path; T2/T3. |
| **G3** | Security / policy screening (`ALLOW/QUARANTINE/REJECT` + detector provenance) | **CODE-COMPLETE (T2/T3-UNVERIFIED)** for the record; **PARTIAL** for coverage | mig 54 `screening_decisions` (applied) + `screening.py`; **now invoked** by `compile_skill_artifact` (one row per finding, alongside the existing downgrade). Remaining: a screen `REJECT` does not yet abort capture (policy deferred); PII/license/malware detectors; SSRF check on locators; block-level redaction; T13. |
...
| **G7** | Procedure extraction / validation / versioning | **CLOSED** *(trace path, offline)* / **PARTIAL** *(overall)* | Document path still has no groundedness validator; `ingestion_jobs.py` placeholder goal (B15) untouched. |
| **G8** | Procedure↔Claim typed refs + applicability integration | **CODE-COMPLETE (T2/T3-UNVERIFIED)** | mig 52 `procedure_claim_refs` (applied) + `procedure_claim_refs.py` + **B5 role-aware invalidation** in `claim_impact.py`; legacy scan kept as compat. **Document path now authors a `RATIONALE` ref** on the derived Claim (`bb2deba`). Remaining: run `backfill_refs_from_preconditions` (A33); wire `add_procedure_claim_ref` into the *trace* extractors; T4. |
| **G9** | Procedure composition validation | **PARTIAL** | Unchanged. |
| **G10** | Implementation Registry validation + Procedure↔Implementation M:N relation metadata | **CODE-COMPLETE (T2/T3-UNVERIFIED)** | mig 53 (applied) generalizes `procedure_implementations`; `procedure_implementations.py`; `solution_implementations.py` reads it, unioned with the legacy path. Remaining: converge `implementation_tasks`; board note for the `schema.md` 1:1↔M:N discrepancy (B17); T6. |
