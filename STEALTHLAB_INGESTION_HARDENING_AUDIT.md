...
- **T3** — no end-to-end golden ingestion test asserting the full document chain. Now writable; needs authoring against the local DB (`dbtarget local`).
- **A33 corpus backfill** — `backfill_refs_from_preconditions` still not run against real corpus data (local DB is schema-only).
...
| **G1** | `IngestionContext` / provenance manifest | **CODE-COMPLETE (T2/T3-UNVERIFIED)** | mig 51 `ingestion_contexts` (applied) + `ingestion_context.py` + back-links on 5 tables. **Both paths wired:** `compile_skill_artifact` (document) opens/completes one and stamps procedure/observation/evidence/artifact/blocks/claim; `ingestion_jobs.resolve_trace_ingestion_context` (trace) opens one per session, stamps observation → claim → procedure → procedure-evidence. Remaining: T2; the trace context is not `complete()`d (spans many async jobs) — left `open` (documented). |
...
| **G9** | Procedure composition validation (cycle rejection, version-pinned child refs, runtime≠canonical) | **CODE-COMPLETE (T2/T3-UNVERIFIED)** | **Done** (`035adf6`): `procedure_graph.validate_procedure_composition_{definition,in_storage}` — write-time gate rejecting cycles / unresolved pinned refs / max-depth; wired into `capture_procedure` + `supersede_procedure`. Tests: `test_procedure_composition_e2e.py`, `test_procedure_graph_offline.py`. |
