...
| **G12** | Local schema-aligned learning + scope / private sync | **PARTIAL → the write-back AND publish gaps closed** | The cloud private-sync *tier* (A13: a separate local↔hosted change-log/cursor protocol) stays deferred — no dedicated storage for it. Two real, narrower gaps got found and closed instead: (1) G13's `.stealth/` build (P1–P4) added `exploration.md`/`open_exploration`/`close_exploration` as a purely LOCAL, journal-only mechanism with no MCP tool exposing it and no path back to durable storage — **fixed**: `close_exploration` now optionally captures the resolution as a real, private, owner-scoped Claim in Postgres (real provenance via a registered `agent_execution` Source, honest `subject`/`predicate`/`object`); new MCP tools `open_exploration`/`close_exploration` expose this to agents for the first time. (2) There was no explicit private→global **publish** gate for Claims at all (only Procedures had one, `publication.py::publish_procedure`) — a private Claim had no audited path to ever become global except a raw, ungated `UPDATE`. **Fixed**: `app/services/claim_publication.py::publish_claim` — ownership check, the G3 content screen (a block-severity finding refuses), source-lineage check (a claim citing a private Source cannot generalize to public), `_scrub_value` scrubbing, audited via a ChangeSet, fails closed, never automatic — the exact same posture `publish_procedure` already has. `test_claim_publication_e2e.py` proves the full loop: exploration resolves → private Claim → explicit `publish_claim` → global. |
