...
| **G3** | Security / policy screening (`ALLOW/QUARANTINE/REJECT` + detector provenance) | **CODE-COMPLETE (DB-VERIFIED + detector coverage)** | mig 68 `screening_decisions` + `screening.py`, invoked by `compile_skill_artifact`. SSRF locator guard (`3ce9a6d`): `screen_locator`/`assert_safe_locator` wired into both ingestion `_default_http_get` paths, `follow_redirects=False` + per-hop re-screen, aborts on hit. **PII / license / malicious-executable detectors landed** (`db3d597`): SSN + Luhn-checked card shapes + email/phone-list detection (redacted markers only, never raw values) → QUARANTINE; a curated restrictive-license phrase list → QUARANTINE; base64-decode-then-exec dropper shapes (bash/sh/powershell/python/js, pipe-to-shell) → REJECT. `CHECK_TYPES` now fully implemented except `source_trust` (a source-registry judgment, not a per-document pattern — by design). Remaining: record a `screening_decisions` row on an aborted SSRF fetch (needs a pool threaded into fetch orchestration); a content `REJECT` still only warns, doesn't abort capture (policy deferred). |
| **G4** | Observation extraction / validation (+ structural source classification §6) | **PARTIAL → advanced** | Document path emits one `document_procedure` Observation with `ingestion_context_id`. **Source classifier landed** (`a029848`): `source_classification.classify_source_content` (`PROCEDURE/REFERENCE/CLAIM/MIXED/UNKNOWN` + version + confidence) stamped onto every document Observation's `properties.source_classification`. Remaining: Observations per `artifact_block` / block-span citation; an optional quarantine gate when the classifier says `REFERENCE` despite parsed steps. |
| **G5** | Independent Claim normalization, belief, dedup, conflict/family | **PARTIAL** (advanced) | **B7/B10 done**. **B8 done** — `claim_belief.py`: evidence-only belief with independence de-dup + document ceiling 0.5, written via a ChangeSet citing evidence; hooked on `record_claim_evidence` + `relate_claims`. **B9 done** for evidence/relation paths (`status_from_belief`) — but `failure_handlers.py` still writes `claim_status` directly. **Document path now derives one Claim** (`bb2deba`). **Structured triple landed for the document path** (`a4937d9`): the document-Claim's `capture_claim` call now passes `subject=artifact.uri, predicate="documents_procedure_for", object=proposition` — honest structuring of already-known template components, not inference. The trace path (`observations.py::promote_observation_to_claim`) is deliberately left unpopulated (its `statement` is free prose with no separately-known triple) — documented in-code, not silently missing. **New finding:** `knowledge_nodes` has real top-level `subject`/`predicate`/`object` *columns* (mig 21) that `capture_claim`'s INSERT never writes — the triple lives in `properties` only (consistent with how `claim_family.py` already reads it). Follow-up: wire `capture_claim` to also populate the columns. |
| **G6** | Evidence normalization + independence accounting | **PARTIAL → advanced** (`a4937d9`) | **B10 done**. Document path writes a typed `evidence(type='document')` row for the procedure. **A second, per-Claim evidence row now lands too** — `_emit_document_claim_evidence` (`target_type='claim'`, `target_version=NULL`, same `independence_group` keying) — so `claim_belief.py` has real evidence to aggregate for a document-derived Claim instead of sitting on the zero-evidence floor forever. `test_ingestion_canonical_chain_e2e.py` proves the row against the real schema. |
| **G7** | Procedure extraction / validation / versioning | **advanced both paths** (`a4937d9`) | **B15 done** (`035adf6`): trace path requires source-supplied `goal_text`, no fabricated goal/outcome. **Document-path groundedness landed**: `_check_document_groundedness` (step-text overlap with raw content, non-placeholder name/description, step-count sanity vs. the raw list) stamped onto the document Observation's `properties.groundedness` — audit-not-block posture matching G3/G4, never aborts capture. `test_document_groundedness_offline.py` (8). |
...
| **G12** | Local schema-aligned learning + scope / private sync | **PARTIAL → the write-back gap closed** | The cloud private-sync *tier* (A13: a separate local↔hosted change-log/cursor protocol) stays deferred — no dedicated storage for it. But a real, narrower gap got found and closed: G13's `.stealth/` build (P1–P4) added `exploration.md`/`open_exploration`/`close_exploration` as a purely LOCAL, journal-only mechanism with no MCP tool exposing it and no path back to durable storage — a resolved exploration was lost the moment the workspace went away, and could never become a global candidate. **Fixed**: `close_exploration` now optionally captures the resolution as a real, private, owner-scoped Claim in Postgres (real provenance via a registered `agent_execution` Source — not synthetic, not fabricated; honest `subject`/`predicate`/`object` from the already-known question/resolution strings) when given a pool + a real `RESOLVED` outcome; `ABANDONED` or an empty resolution captures nothing (there's nothing learned to keep). New MCP tools `open_exploration`/`close_exploration` expose this to agents for the first time — previously unreachable from any running surface. This is the "local learning → private global candidate" pipeline for the *new* filesystem-based local architecture, replacing the one the removed SQLite `local_learning_sweep.py`/`publish_local_procedure` used to provide. The private Claim sits behind the same explicit-publish gate every other private object does — nothing auto-promotes to global. |
...
| **G14** | Global hierarchical retrieval + index freshness | **CLOSED** (honest-minimal, `4a9ef40`) | No detached index exists — candidates are re-hydrated from live rows before ranking/selection, so the correctness guarantee is structural. mig 72: `procedures.retrieval_indexed_at` + `procedure_index_lag` view; `index_freshness.get_index_lag` (timestamp/embedding lag + service-layer recipe-version drift); backfill script stamps `retrieval_indexed_at`; `find_applicable_procedures` docstring states the index is advisory. `test_index_lag_e2e.py` (2, local). Domain/topic routing (a perf optimisation, not a correctness gap) remains optional. |
...
| **G24** | Publication / privacy / license / IP dependency traversal | **CODE-COMPLETE (offline-only)** | **B11 done** (`acbd2f5`): `publication_deps.traverse_publication_dependencies` walks procedure→claims→observations→sources→artifacts→evidence, fail-closed (private/org, `PRIVATE_CLASSES`, unresolved/low-reliability source, or traversal-bound hit → blocking); "private evidence ≠ global verification" enforced (`verification_inherited` always false; `global_verification_required` unless ≥2 independent public verification groups). Wired into `publish_procedure`; `publication_records` carries the full traversal + verification determination. **DB E2E landed** (`a029848`): `test_publication_deps_e2e.py` (2, local) — traversal SQL matches the live schema, private Source in a claim lineage blocks, all-public does not. Remaining: independent *global* re-verification is recorded-as-required but not executed; license/IP checks are visibility/classification-based only. |
...
| T13 | **PARTIAL → advancing** — `test_screen_locator_offline.py` (23): file:// / metadata IPs+hostnames / all RFC-1918 / ::1 / 0.0.0.0 / DNS-to-loopback rejected; github/public hosts pass; DNS failure denies not crashes; `LocalDirSkillSource` skips a symlink escaping the root (skips on win32, runs in CI). Prior T13 coverage (path traversal, auth spoof, cross-workspace, prompt injection) unchanged. Remaining: replayed-approval hardening. |
| T14 | **advanced** (`15954d0`) — `test_latency_e2e.py`: p50/p95/p99 over real wall-clock calls to `find_applicable_procedures` (p50≈98ms/p95≈143ms/p99≈246ms on a 40-row local corpus) and `HybridRetriever.retrieve` (p50≈24ms/p95≈54ms/p99≈103ms), plus a floor-cost sanity check — real Postgres, no mocking, honestly scoped as service-call latency (no running MCP-server network harness exists in this codebase to benchmark against). Ceilings are documented regression trip-wires with 10x+ headroom, not hardware guarantees. |
