...
| **G3** | Security / policy screening (`ALLOW/QUARANTINE/REJECT` + detector provenance) | **CODE-COMPLETE (DB-VERIFIED)** for the record; **PARTIAL** for coverage | mig 68 `screening_decisions` + `screening.py`, invoked by `compile_skill_artifact`. **SSRF locator guard added** (`3ce9a6d`): `screen_locator`/`assert_safe_locator` (scheme + DNS-resolved-IP classification) wired into both ingestion `_default_http_get` paths with `follow_redirects=False` + per-hop re-screen; an SSRF attempt aborts the fetch. Remaining: PII/license/malware detectors; record a `screening_decisions` row on an aborted fetch (needs a pool in the fetch orchestration); a content `REJECT` still only warns. |
...
| T14 | **OPEN** — latency p50/p95/p99 rig. |
