...
from app.execution.executor_kinds import ExecutorKindViolation
...
def test_step_binding_is_validated_and_carried_onto_the_node():
    steps = [{"order": 0, "goal": "run the linter", "binding": {"kind": "command", "command": "make lint"}}]
    nodes = steps_to_linear_nodes(steps)
    assert nodes[0].binding == {"kind": "command", "command": "make lint"}
    assert nodes[0].executor_hint == ("deterministic",)


def test_step_with_no_binding_yields_none_not_a_default():
    nodes = steps_to_linear_nodes([{"order": 0, "goal": "no binding here"}])
    assert nodes[0].binding is None and nodes[0].executor_hint is None


def test_invalid_binding_is_rejected_at_the_gate_not_silently_accepted():
    from app.services.source_locators import SourceLocatorError
    with pytest.raises(SourceLocatorError):
        steps_to_linear_nodes([{"order": 0, "goal": "do the work", "binding": {"kind": "quantum_hivemind", "quantum_hivemind": "x"}}])


def test_legacy_implementation_hint_key_is_ignored():
    nodes = steps_to_linear_nodes([{"order": 0, "goal": "old row", "implementation_hint": "slm"}])
