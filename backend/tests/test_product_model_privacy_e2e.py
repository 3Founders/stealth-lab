...
A private Goal already stays private through `scope_predicates()`. Its
downstream object graph did NOT: `get_benchmark` / `list_goal_benchmarks`
/ `get_evaluation` / `list_goal_evaluations` ran raw, unscoped queries, so
another user could read/list a private Goal's Benchmark and Evaluation
data directly or infer it through the leaderboard.

Fix: those accessors now inherit the owning Goal's visibility (the exact
rule `list_goal_solutions` already applies), the REST routes thread the
...
async def _full_lineage(pool, *, visibility: str, owner_id: str | None, tag: str):
    """Goal -> frozen Benchmark -> Solution -> completed Evaluation, all
    through the real product_model write paths. Returns the id bundle. The
    `objective` carries a tag-unique nonce so a lexical `find_goal`
    match can only ever be THIS goal (the shared live DB holds hundreds
    of residual test goals)."""
    ...
    goal = await find_or_create_goal(
        pool, canonical_name=f"[{_PREFIX} {tag}] {visibility} goal",
        scope_type="global", provenance="system_pending_review",
        objective=objective, created_by=owner_id or "anon",
        visibility=visibility, owner_id=owner_id, on_unavailable="create",
    )
    bench = await pm.create_benchmark(
        pool, goal_id=goal["id"], name=f"{_PREFIX}-bench-{tag}", version=1,
    ...
    return {
        "goal_id": goal["id"], "benchmark_id": bench["id"],
...
async def test_private_benchmark_and_evaluation_inherit_problem_scope_service_layer():
    ...
    try:
        ...
        assert await pm.list_goal_benchmarks(pool, priv["goal_id"], scope=alice)
        assert await pm.get_evaluation(pool, priv["evaluation_id"], scope=alice) is not None
        assert await pm.list_goal_evaluations(pool, priv["goal_id"], scope=alice)
        lb_owner = await pm.goal_leaderboard(pool, priv["goal_id"], scope=alice)
        ...
        for other in (bob, anon):
            ...
            assert await pm.list_goal_benchmarks(pool, priv["goal_id"], scope=other) == []
            assert await pm.get_evaluation(pool, priv["evaluation_id"], scope=other) is None
            assert await pm.list_goal_evaluations(pool, priv["goal_id"], scope=other) == []
            lb = await pm.goal_leaderboard(pool, priv["goal_id"], scope=other)
        ...
        # --- a PUBLIC goal's downstream graph is still readable by anyone ---
        ...
        for who in (bob, anon):
            ...
            assert await pm.list_goal_benchmarks(pool, pub["goal_id"], scope=who)
            assert await pm.get_evaluation(pool, pub["evaluation_id"], scope=who) is not None
            lb = await pm.goal_leaderboard(pool, pub["goal_id"], scope=who)
...
async def test_private_benchmark_and_evaluation_denied_cross_user_over_rest():
    ...
    try:
        ...
        bid, eid, pid_ = priv["benchmark_id"], priv["evaluation_id"], priv["goal_id"]
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://t") as c:
            ...
            assert (await c.get(f"/v1/goals/{pid_}/benchmarks", headers=a)).json()["benchmarks"]
            assert (await c.get(f"/v1/evaluations/{eid}", headers=a)).status_code == 200
            assert (await c.get(f"/v1/goals/{pid_}/evaluations", headers=a)).json()["evaluations"]
            assert (await c.get(f"/v1/goals/{pid_}/leaderboard", headers=a)).status_code == 200

            # bob: denied / empty, and no inference leak in the bodies
            assert (await c.get(f"/v1/benchmarks/{bid}", headers=b)).status_code == 404
            rb = await c.get(f"/v1/goals/{pid_}/benchmarks", headers=b)
            ...
            re = await c.get(f"/v1/goals/{pid_}/evaluations", headers=b)
            assert re.status_code == 200 and re.json()["evaluations"] == []
            assert (await c.get(f"/v1/goals/{pid_}/leaderboard", headers=b)).status_code == 404
            # the goal itself is already 404 to bob (unchanged behaviour)
            assert (await c.get(f"/v1/goals/{pid_}", headers=b)).status_code == 404
            ...
            assert (await c.get(f"/v1/goals/{pid_}/benchmarks")).json()["benchmarks"] == []
    finally:
        await pool.close()
