"""
MOVED into backend/ (from experiments/swebench_pro/agent.py) so backend/
has zero runtime dependency on the sibling experiments/ tree -- previously
reached via a bare `sys.path.insert` + `from agent import ...`, which is
also what caused a real production incident: with no local `agent` module
on the path, Python silently resolved the bare name to an unrelated,
already-installed global package named `agent` and crashed on an
incompatible dependency deep inside it. Importing this as
`app.execution.coding_agent` makes that class of collision structurally
impossible. Content otherwise unchanged; only this file's location and
import path moved. `experiments/swebench_pro/agent.py` still exists
(historical/research copy) but is no longer imported by anything in
`backend/`.
