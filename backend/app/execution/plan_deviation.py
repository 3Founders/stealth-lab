...
B17: "Persist separately: planned procedure/step bindings AND
actual procedure/version/bindings/steps/tools/artifacts/outcome.
...
def _binding_key(b) -> Optional[str]:
    """A comparable identity for a step binding (its kind + primary value), or None when unbound."""
    if isinstance(b, str):
        b = json.loads(b)
    if not b:
        return None
    kind = b.get("kind")
    return f"{kind}:{b.get(kind) or b.get('entrypoint') or ''}"
...
async def compute_plan_deviation(pool: asyncpg.Pool, execution_run_id: str) -> Optional[dict[str, Any]]:
    ...
    `per_node` entries: {node_order, planned_goal, planned_binding,
    actual_status, actual_binding, attempt_count, error_class,
    ...
    actual_rows = await pool.fetch(
        "SELECT node_order, status, binding, attempt_count, error_class "
    ...
    for row in actual_rows:
        ...
        planned_bind = _binding_key(planned.get("binding"))
        actual_bind = _binding_key(row["binding"])
        ...
        if planned_bind and actual_bind and planned_bind != actual_bind:
            deviations.append("binding_diverged_from_plan")
        ...
        per_node.append({
            ...
            "planned_binding": planned_bind,
            "actual_status": row["status"],
            "actual_binding": actual_bind,
    ...
    for order in planned_nodes:
        if order not in {n["node_order"] for n in per_node}:
            per_node.append({
                ...
                "planned_binding": None, "actual_status": None,
                "actual_binding": None, "attempt_count": 0, "error_class": None,
