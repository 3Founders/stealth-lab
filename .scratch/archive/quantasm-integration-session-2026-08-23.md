...
> **Status: ARCHIVED 2026-08-25.** This session summary was formerly the
> entire content of `commLLM.md`, which has since been repurposed as the
> system-wide consolidated reference. The integration plan itself remains
> live, with one amendment: the "backfill runs/*.jsonl" step in the Week-1
> sequence is **superseded by ROADMAP's fresh-start ruling** (no data
> backfills, ever). When QuantAsm integration resumes, its rollouts enter
> through the normal trace-ingestion boundary (`/v1/traces` → V0 gate),
> labeled synthetic/QuantAsm by provenance — same rules as any other
> producer. Nothing here was silently dropped; it moved.
