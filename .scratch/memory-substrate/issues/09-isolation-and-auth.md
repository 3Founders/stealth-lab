...
the trace-header table from ticket 06, anything for claims/procedures) gets **both `owner_id TEXT`
and `visibility visibility_level NOT NULL DEFAULT 'public'`** from row one — not deferred "for
later" the way `tenant_id` was. Every new query goes through `app/services/access.py`'s
`visibility_predicate()` — no hand-written filters, no exceptions.
...
**Correction (added on review — an earlier draft of this answer said `owner_id` alone).** The two
columns are not independently satisfiable requirements; they are a pair, and naming only
`owner_id` while also mandating `visibility_predicate()` was internally inconsistent.
`visibility_predicate()` emits `visibility = 'public'` in *every* non-unrestricted branch
(`access.py:83-93`), so a table carrying `owner_id` but no `visibility` column would generate SQL
referencing a column that does not exist — failing at query time while passing every check before
it. That is precisely the `embedding_joint` ghost-column failure shape ticket 17 identifies as a
live drift in this repo, recreated inside this ticket's own prescription. The real schema always
adds them together: `db/03_access.sql:28-43` applies the identical
`visibility ... NOT NULL DEFAULT 'public'` + `owner_id TEXT` pair to all four tables it covers,
with partial indexes on each.

**A related gap, previously unstated:** `traces` and `episodes` currently have **neither** column
— `03_access.sql` covers only `knowledge_nodes`, `task_nodes`, `edges` and `debates`. The two
existing tables closest to this effort's subject matter sit entirely outside the access-control
system today. This does not change the decision above (new tables carry both columns from row
one), but anyone assuming "existing tables are already covered by `access.py`" would be wrong for
exactly the two tables this effort touches most.
